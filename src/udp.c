...
udp_err_t udp_recvmmsg(void* buf, size_t n, size_t pack_size, udp_session_t* session)
{
...
#ifdef DEBUG
    printf("[udp_recvmmsg] n=%zu, pack_size=%zu, batch_size=%d, remainder=%zu\n",
           n, pack_size, batch_size, remainder);
#endif
    ...
    while (total_received < batch_size) 
    {
...
#ifdef DEBUG
        printf("[udp_recvmmsg] recvmmsg returned %d (total_received=%d / batch_size=%d)\n",
               received, total_received, batch_size);
#endif

        if (received < 0)
        {
#ifdef DEBUG
            printf("[udp_recvmmsg] recvmmsg failed: errno=%d (%s)\n", errno, strerror(errno));
#endif
            return UDP_RECV_FAIL;
        }
        ...
        {
#ifdef DEBUG
            printf("[udp_recvmmsg] recvmmsg returned 0, potential infinite loop risk\n");
#endif
        }
...
#ifdef DEBUG
    printf("[udp_recvmmsg] batch complete: received %d packets\n", total_received);
#endif

    if (remainder > 0)
    {
#ifdef DEBUG
        printf("[udp_recvmmsg] receiving remainder: %zu bytes at offset %zu\n",
               remainder, batch_size * pack_size);
#endif
    
        if (udp_recv((uint8_t*)buf + batch_size * pack_size, remainder, session) != UDP_OK)
        {
#ifdef DEBUG
            printf("[udp_recvmmsg] udp_recv for remainder failed\n");
#endif
            return UDP_RECV_FAIL;
        }
    
#ifdef DEBUG
        printf("[udp_recvmmsg] remainder received successfully\n");
#endif
    
    }

#ifdef DEBUG
    printf("[udp_recvmmsg] all data received successfully\n");
#endif
